...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        const result = await axios.get("http://localhost:5000/auth/profile", { withCredentials: true });
        if (result.data.Status) {
          setAdmin(result.data.Result); // Now using only logged-in user data
        }
      } catch (error) {
        console.error("Error fetching admin data:", error);
      }
